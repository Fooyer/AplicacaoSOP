...
def allocate_memory_best_fit(size):
    best_fit_block = None
    ...
    if best_fit_block is not None:
        ...
        if best_fit_block.size - size > 0:
            new_block = MemoryBlock(best_fit_block.start_address + size, best_fit_block.size - size)
            memory_blocks.append(new_block)
            best_fit_block.size = size
        return best_fit_block.start_address
...
def alocarMemoria_b(valor):
    allocated_address = allocate_memory_best_fit(valor) 
...
def desalocarMemoria_b(endereco):
