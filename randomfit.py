...
# Função para alocar memória aleatoriamente
def allocate_memory_random(size):
    available_blocks = [block for block in memory_blocks if not block.allocated and block.size >= size]
...
def alocarMemoria_r(valor):
    allocated_address = allocate_memory_random(valor)
...
def desalocarMemoria_r(endereco):
